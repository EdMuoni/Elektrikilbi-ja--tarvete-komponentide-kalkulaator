...
add(
  ...
  figure(String.raw`Diagrams\ERD_ElektriKalkulaator.png`,
         "Realiseeritud andmemudeli olemi-suhte diagramm (ERD). Joonis on koostatud EF Core mudelist, mitte joonistatud käsitsi — seetõttu vastab see täpselt sellele, mis andmebaasis tegelikult on. Katkendjoon näitab ainsat seost, mida andmebaasis võtmena ei ole.",
         580),
...
add(
  ...
  h2("Kavandatud ja realiseeritud andmemudeli vahe"),
  p("Enne kirjutamise alustamist koostati andmemudeli kavand. See oli märgatavalt laiem kui see, mis nädalate pärast valmis sai. Kavandit ei ole põhjust ära peita — vahe kavandatu ja valminu vahel on üks selle töö sisulisi tulemusi."),
  figure("Whole_Building_Electrical_ERD.png",
         "Kavandamise etapis koostatud andmemudel. Realiseeritud mudel (joonis eespool) on selle kitsam alamhulk.", 580, DRAFT),
  p("Kavandis olid muu hulgas eraldi tabelid tootekirjelduste tõlgete jaoks (PRODUCT_TRANSLATION), toote tehniliste näitajate jaoks (PRODUCT_SPEC), tootjate jaoks (BRAND) ning kolm tabelit tehisintellekti teenusega suhtlemiseks (AI_SERVICE, AI_REQUEST, AI_RESPONSE)."),
  p("Valminud lahenduses on neist realiseeritud kasutajapõhine arvutuste ajalugu. Tootja jäi eraldi tabeli asemel Products-tabeli veeruks Brand, sest kataloogis on kolm tootjat ja eraldi tabel oleks lisanud liitmise ilma ühegi kasuta. Ülejäänu jäi edasiarenduseks."),
  p("Peamine õppetund kavandamise kohta oli see, et andmemudeli laius tuleb siduda sellega, mida jõuab valmis ehitada. Liiga lai kavand ei ole kasutu, sest see näitab suunda, kuid selle täielik realiseerimine ei oleks 156 tunni sisse mahtunud — ja pooleliolev tabel andmebaasis on halvem kui tabel, mida üldse ei ole."),
  p("Just seetõttu on realiseeritud mudel teadlikult kitsam: kuus tabelit, mis katavad arvutuse, tootekataloogi ja arvutuste salvestamise, ning nende kõrval ASP.NET Core Identity enda tabelid. Iga neist on täielikult töös, mitte poolikult."),

  h2("Soovitused sarnase töö tegijale"),
  p("Kolm soovitust, mis tulenevad otseselt selle töö kogemusest ja mida autor oleks tahtnud alguses teada."),
  rich([{ t: "Alusta turvalisusest, ära lõpeta sellega. ", b: true }, { t: "Autentimise ja rollide lisamine puudutab korraga kontrollereid, vaateid ja andmemudelit. Kui see jätta lõppu, tuleb juba valmis osi ümber teha — täpsemalt nii selles töös läkski." }]),
  rich([{ t: "Kirjuta iga muudatuse juurde selle põhjus. ", b: true }, { t: "Kood ise näitab, mis muutus. Põhjust ei näita miski peale kirjapandu, ja nädalase pausi järel on just põhjus see, mida tegelikult vaja läheb." }]),
  rich([{ t: "Kontrolli töötavat rakendust, mitte ainult kompilaatorit. ", b: true }, { t: "Selle töö kõige aeganõudvamad vead olid kõik süntaktiliselt korrektsed: valge tabel tumedal taustal, nähtamatud pealkirjad ja märkeruut, mis midagi ei teinud. Ükski neist ei andnud ehitamisel häiret ja üksi kompilaatorit usaldades oleks kõik kolm töösse jäänud." }]),
  new Paragraph({ children: [new PageBreak()] }),
